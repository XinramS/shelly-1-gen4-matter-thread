...
- Additional variants. The Light and Opener variants are released. A Switch variant is planned: a latching relay like Light, with the SW terminal kept as a physical wall toggle, presented as a switch or outlet rather than a light. This differs from the Opener, which uses a momentary pulse and dedicates the SW terminal to a contact sensor. A Light Switch variant, a Matter On/Off Light Switch that controls other Matter lights, is also planned.
