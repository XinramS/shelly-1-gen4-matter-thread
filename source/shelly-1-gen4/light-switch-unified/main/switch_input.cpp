...
while (xQueueReceive(switch_evt_queue, &io_num, 0) == pdTRUE) {}
        ...
        ESP_LOGI(TAG, "SW input changed. Toggling relay and sending bound Toggle command");

        lock::ScopedChipStackLock lock(portMAX_DELAY);

        // 1. Flip the relay locally by writing the Light endpoint's OnOff
        //    attribute. app_driver_attribute_update() (PRE_UPDATE callback)
        //    picks this up and calls relay_set() for us — same path a
        //    HomeKit/HA toggle would take, so state stays in sync and gets
        //    reported to every fabric.
        ...
        // 2. Unchanged: fire Toggle to whatever's bound to switch_endpoint_id
